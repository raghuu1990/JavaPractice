...
public class SegmentRangeSum {
	...
	public static void main(String args[]) {
		...
		System.out.println("Sum of values in range [" + qs + ", " + qe + "] is = " + rangeSumQuery(n, qs, qe));
		updateValue(arr, n, 1, 10);
		System.out.println("Updated sum of values in range [" + qs + ", " + qe + "] is = " + rangeSumQuery(n, qs, qe));
	}
	...
	public static int rangeSumQuery(int n, int qs, int qe) {
	...
	public static int rangeSumQueryUtil(int ss, int se, int qs, int qe, int si) {
		if (qs <= ss && qe >= se) {
			return st[si];
		}
		...
		int mid = getMid(ss, se);
		return rangeSumQueryUtil(ss, mid, qs, qe, 2 * si + 1) + rangeSumQueryUtil(mid + 1, se, qs, qe, 2 * si + 2);
	}
